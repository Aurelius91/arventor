	...
	<script type="text/javascript">
					...
					$.ajax({
						...
						success: function(data){
							if (data.status == 'success') {
								$('#purchase-product-price-'+ number).val(0);
		...
		function reset() {
			$('#purchase-number').val("");
			$('#purchase-date').val("<?= $date_display; ?>");
			$('.purchase-item-quantity').val('1');
			$('.purchase-item-price').val('0');
			$('#purchase-discount').val('0');
			$('#purchase-shipping').val('0');
			$('#purchase-tax').val('0');

			<? if ($account->location_id > 0): ?>
				$('#purchase-location').val("<?= $account->location_id; ?>");
				$('#purchase-location-selection').dropdown('set selected', "<?= $account->location_id; ?>");
			<? else: ?>
				$('#purchase-location').val("<?= $arr_location[0]->id; ?>");
				$('#purchase-location-selection').dropdown('set selected', "<?= $arr_location[0]->id; ?>");
			<? endif; ?>

			$('#purchase-vendor').val("<?= $setting->setting__webshop_default_vendor_id; ?>");
			$('#purchase-vendor-selection').dropdown('set selected', "<?= $setting->setting__webshop_default_vendor_id; ?>");

			$('#purchase-method').val("Cash");
			$('#purchase-method-selection').dropdown('set selected', "Cash");

			$('#purchase-statement').val("<?= $arr_statement[0]->id; ?>");
			$('#purchase-statement-selection').dropdown('set selected', "<?= $arr_statement[0]->id; ?>");
		}
						...
						<div class="field">
							<div class="three fields">
								...
								<div class="field">
									...
									<div id="purchase-method-selection" class="ui search selection dropdown form-input">
							...
							<div class="field">
								<table class="ui striped selectable celled table" style="border: 1px solid rgba(34, 36, 38, 0.15); border-radius: 0;">
									...
									<tbody id="purchase-item-list">
										<tr id="purchase-item-list-1" class="purchase-product-list" data-number="1">
											<td class="td-icon">
											...
											<td class="td-price-quantity" style="text-align: right;">
												<input id="purchase-product-price-1" type="text" class="purchase-item-price" data-number="1" placeholder="Price.." style="text-align: right;">
											</td>
											<td class="td-price-quantity" style="text-align: right;">
												<input id="purchase-product-quantity-1" type="text" class="purchase-item-quantity" data-number="1" placeholder="Quantity.." style="text-align: right;">
											</td>
											<td class="td-price-quantity" style="text-align: right;">
												<span id="purchase-product-price-total-1" data-number="1" data-total="0">Rp 0</span>
											</td>
										</tr>
										<tr>
